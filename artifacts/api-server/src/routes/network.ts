    ...
    const rules = [
      {
        ...
        hitCount: 2847291,
        riskLevel: "low",
        aiAnalysis: "Standard HTTPS ingress rule — properly scoped to web server. No issues detected.",
        issues: [],
        recommendation: null,
      ...
      {
        id: 2,
        ruleName: "ALLOW-ANY-ANY-ALL-PORTS",
        ruleId: "FW-2023-0007",
        chain: "INPUT",
        action: "ALLOW",
        sourceIp: "0.0.0.0/0",
        ...
        direction: "inbound",
        status: "active",
        createdAt: new Date(now - 86400000 * 730).toISOString(),
        lastHit: new Date(now - 60000).toISOString(),
        hitCount: 18472910,
        riskLevel: "critical",
        aiAnalysis: "Overly permissive rule allowing ALL traffic from ANY source to ALL ports. This effectively disables the firewall for inbound traffic and should be removed or replaced with specific allow rules.",
        issues: [
          { type: "overly_permissive", severity: "critical", detail: "Rule allows all ports (1-65535) from any source — equivalent to no firewall" },
          { type: "no_source_restriction", severity: "critical", detail: "Source 0.0.0.0/0 allows traffic from the entire internet" },
          { type: "stale_rule", severity: "high", detail: "Rule created 2 years ago — may be a leftover from initial setup or debugging" },
        ],
        recommendation: "Remove this rule immediately and replace with specific port-based allow rules for required services only.",
        category: "misconfiguration",
      },
      {
        id: 3,
        ruleName: "ALLOW-DEV-SSH-ANY",
        ruleId: "FW-2024-0089",
        chain: "INPUT",
        action: "ALLOW",
        sourceIp: "0.0.0.0/0",
        ...
        lastHit: new Date(now - 7200000).toISOString(),
        hitCount: 89421,
        riskLevel: "high",
        aiAnalysis: "SSH access open to entire internet. While SSH is encrypted, this exposes the service to brute force attacks. Should be restricted to VPN or bastion host IPs only.",
        issues: [
          { type: "overly_permissive", severity: "high", detail: "SSH port 22 exposed to 0.0.0.0/0 — should be restricted to VPN subnet 10.8.0.0/16" },
          { type: "brute_force_risk", severity: "high", detail: "14,235 failed SSH attempts detected from this rule in the last 24 hours" },
        ],
        recommendation: "Restrict source to VPN subnet (10.8.0.0/16) or bastion host IP. Enable fail2ban on all SSH-enabled hosts.",
        category: "access_control",
      },
      {
        id: 4,
        ruleName: "SHADOW-IT-DROPBOX-SYNC",
        ruleId: "FW-DETECTED-001",
        chain: "OUTPUT",
        action: "ALLOW",
        sourceIp: "10.0.3.0/24",
        destinationIp: "162.125.0.0/16",
        port: "443",
        protocol: "TCP",
        direction: "outbound",
        status: "active",
        createdAt: new Date(now - 86400000 * 90).toISOString(),
        lastHit: new Date(now - 1800000).toISOString(),
        hitCount: 45872,
        riskLevel: "high",
        aiAnalysis: "Detected Shadow IT: Finance department (10.0.3.0/24) has active Dropbox sync traffic to Dropbox IP range. Dropbox is not an approved file-sharing platform — corporate policy requires OneDrive/SharePoint.",
        issues: [
          { type: "shadow_it", severity: "high", detail: "Unauthorized cloud storage (Dropbox) usage detected — 3.2 GB synced in last 7 days" },
          { type: "data_exfiltration_risk", severity: "high", detail: "Finance department data flowing to unapproved cloud provider" },
        ],
        recommendation: "Block Dropbox IP ranges (162.125.0.0/16) and notify Finance department to migrate to approved OneDrive storage.",
        category: "shadow_it",
      },
      {
        id: 5,
        ruleName: "DENY-ALL-DEFAULT",
        ruleId: "FW-2024-DEFAULT",
        chain: "INPUT",
        action: "DENY",
        sourceIp: "0.0.0.0/0",
        destinationIp: "0.0.0.0/0",
        ...
        hitCount: 47291847,
        riskLevel: "low",
        aiAnalysis: "Default deny rule — correctly positioned as last rule in chain. This is the expected baseline security posture.",
        issues: [],
        recommendation: null,
        category: "baseline",
      },
      {
        id: 6,
        ruleName: "ALLOW-INTERNAL-DB-WIDE",
        ruleId: "FW-2024-0156",
        chain: "INPUT",
        action: "ALLOW",
        sourceIp: "10.0.0.0/8",
        ...
        hitCount: 1284729,
        riskLevel: "medium",
        aiAnalysis: "PostgreSQL access rule is too broad — allows any internal host (10.0.0.0/8) to connect to the production database. Should be restricted to application server IPs only.",
        issues: [
          { type: "overly_permissive", severity: "medium", detail: "Entire 10.0.0.0/8 subnet can access production database — only app servers (10.0.1.15, 10.0.1.16) need access" },
          { type: "lateral_movement_risk", severity: "medium", detail: "Compromised workstation could directly access production database" },
        ],
        recommendation: "Restrict source to specific application server IPs: 10.0.1.15/32, 10.0.1.16/32",
        category: "access_control",
      },
      {
        id: 7,
        ruleName: "ALLOW-CRYPTO-MINING-POOL",
        ruleId: "FW-DETECTED-002",
        chain: "OUTPUT",
        action: "ALLOW",
        sourceIp: "10.0.2.1",
        destinationIp: "104.243.33.0/24",
        port: "3333,4444,8333",
        protocol: "TCP",
        direction: "outbound",
        status: "active",
        ...
        hitCount: 89241,
        riskLevel: "critical",
        aiAnalysis: "CRITICAL: Outbound traffic to known cryptocurrency mining pool ports (3333/4444/8333) from production database server. This is either unauthorized mining software or a compromised server.",
        issues: [
          { type: "malicious_rule", severity: "critical", detail: "Ports 3333, 4444, 8333 are standard cryptocurrency mining pool ports (Stratum protocol)" },
          { type: "compromised_server", severity: "critical", detail: "Production database server (10.0.2.1) communicating with known mining pool IP range" },
          { type: "unauthorized_modification", severity: "high", detail: "Rule created 5 days ago with no change request ticket — possible unauthorized access" },
        ],
        recommendation: "Block immediately. Investigate production database server for compromise. Check for unauthorized access to firewall management console.",
        category: "malicious",
      },
      {
        id: 8,
        ruleName: "ALLOW-LEGACY-TELNET",
        ruleId: "FW-2019-0003",
        chain: "INPUT",
        action: "ALLOW",
        sourceIp: "10.0.5.0/24",
        destinationIp: "10.0.6.0/24",
        port: "23",
        protocol: "TCP",
        direction: "inbound",
        status: "active",
        createdAt: new Date(now - 86400000 * 1825).toISOString(),
        lastHit: new Date(now - 86400000 * 90).toISOString(),
        hitCount: 127,
        riskLevel: "high",
        aiAnalysis: "Legacy Telnet rule active for 5 years with minimal recent usage (last hit 90 days ago). Telnet transmits credentials in plaintext and should be replaced with SSH.",
        issues: [
          { type: "insecure_protocol", severity: "high", detail: "Telnet (port 23) transmits all data including credentials in cleartext" },
          { type: "stale_rule", severity: "medium", detail: "Only 127 hits total with no usage in 90 days — likely abandoned" },
        ],
        recommendation: "Disable this rule and migrate any remaining Telnet-dependent systems to SSH. Verify no active Telnet sessions exist.",
        category: "legacy",
      },
    ];

    const totalRules = rules.length;
    const criticalIssues = rules.filter((r) => r.riskLevel === "critical").length;
    const highIssues = rules.filter((r) => r.riskLevel === "high").length;
    const cleanRules = rules.filter((r) => r.issues.length === 0).length;
    const shadowItDetected = rules.filter((r) => r.category === "shadow_it").length;
    const totalIssues = rules.reduce((s, r) => s + r.issues.length, 0);

    res.json({
      rules,
      summary: { totalRules, criticalIssues, highIssues, cleanRules, shadowItDetected, totalIssues },
    });
  } catch (err: any) {
    console.error("[network] GET /firewall-rules failed:", err.message);
    res.status(500).json({ error: "Failed to retrieve firewall rule analysis." });
  }
});
